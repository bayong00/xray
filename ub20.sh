...
IMP="wget -q -O"
local_date="/usr/bin/"
myhost="https://raw.githubusercontent.com/bayong00/xray/main/file_xtls/"
...
judge() {
  if [[ 0 -eq $? ]]; then
    print_ok "$1 Complete... by rioxy | thx to ${Green}bhoikfostyahya${Font}"
    sleep 1
  else
...
# Close wgcf-warp to prevent misjudgment of VPS IP situation
  ...
  if [[ ${domain_ip} == "${local_ipv4}" ]]; then
    ...
    print_error "Please make sure that the correct A records are added to the domain name, otherwise xray will not work properly"
    print_error "The IP address of the domain name resolved through DNS does not match the IPv4 address of the machine, continue installed successfully?（y/n）" && read -r install
    case $install in
    [yY][eE][sS] | [yY])
...
function dependency_install() {
  ...
  mkdir /usr/local/premium-vps
  useradd $mod
  echo "$mod" > /usr/local/premium-vps/${mod}
...
function install_xray() {
 ...
 # // Make Folder Xray & Import link for generating Xray | rioxy AUTOSCRIPT
...
# set uuid
uuid="9b4fb053-dc7c-472e-864e-4d18df4436d7"
# xray config
...
NC='\e[0m'
echo -e "$green ¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶ $NC"
echo -e "$green ¶ Autoscript MOD by rioxy ¶ $NC"
echo -e "$green ¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶¶ $NC"
